...
import com.gnn.seckill.rabbitmq.MiaoshaMessage;
import com.gnn.seckill.redis.RedisService;
import com.gnn.seckill.redis.prefix.GoodsKey;
import com.gnn.seckill.redis.prefix.MiaoshaKey;
...
//		}
     ...
     */
    @AccessLimit(seconds = 5, maxCount = 5, needLogin = true)
    @RequestMapping(value="/test_miaosha", method= RequestMethod.POST)
    @ResponseBody
    public Result<Integer> testMiaosha(Model model, MiaoshaUser user,
                                   @RequestParam("goodsId") long goodsId) {
        Result<Integer> result = Result.build();
